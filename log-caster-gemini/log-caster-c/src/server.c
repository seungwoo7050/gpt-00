...
static void handle_client_job(void* arg) {
    ...
    while ((bytes_read = recv(job->client_fd, buffer, BUFFER_SIZE - 1, 0)) > 0) {
        ...
        // [SEQUENCE: 413] Enforce reasonable log size limit
        ...
        // [SEQUENCE: 351] Write to persistent storage if enabled
    ...
    printf("Client handler completed for %s:%d (remaining clients=%d)\n",
...
log_server_t* server_create(int port) {
    ...
    // [SEQUENCE: 353] Initialize persistence to NULL (set by main if enabled)
    server->persistence = NULL;
    
    // [SEQUENCE: 401] Initialize client count mutex
...
void server_destroy(log_server_t* server) {
    ...
    // [SEQUENCE: 352] Cleanup persistence manager
    ...
    // [SEQUENCE: 402] Destroy client count mutex
...
int server_init(log_server_t* server) {
    ...
    printf("LogCaster-C MVP2 server ready (Thread Pool: %d threads)\n", 
...
void handle_new_connection(log_server_t* server) {
    ...
    // [SEQUENCE: 404] Thread-safe client count check
    ...
    server->client_count++;
    int current_clients = server->client_count;
    pthread_mutex_unlock(&server->client_count_mutex);
    ...
    if (!job) {
        perror("malloc");
        close(new_fd);
        // [SEQUENCE: 405] Decrement count on allocation failure
        pthread_mutex_lock(&server->client_count_mutex);
        server->client_count--;
        pthread_mutex_unlock(&server->client_count_mutex);
        return;
    ...
    if (thread_pool_add_job(server->thread_pool, handle_client_job, job) != 0) {
        ...
        free(job);
        close(new_fd);
        // [SEQUENCE: 406] Decrement count on submit failure
        pthread_mutex_lock(&server->client_count_mutex);
        server->client_count--;
        pthread_mutex_unlock(&server->client_count_mutex);
        return;
